(function(undefined) {
  ...
  sigma.prototype.toSVG = function(params) {
    ...
    const w = params.width || DEFAULTS.size;
    const h = params.height || DEFAULTS.size;
    ...
    const labelThreshold = window.network.settings("labelThreshold");

    window.network.graph.nodes().forEach(node => {
      svg
        .querySelector(`[data-label-target="${node.id}"]`)
        .setAttribute("font-weight", "bold");

      if (
        node[`renderer${window.network.renderers[0].conradId}:size`] < // needs refactor
        labelThreshold // not the best practise
      ) {
        svg.querySelector(`[data-label-target="${node.id}"]`).remove();
      }
    });
