(function(undefined) {
   ...
   */
  sigma.parsers.gexf = function(target, sig, callback, err) {
    let i;
    let l;
    let arr;
    let obj;
    ...
    if (typeof target === "string") gexf.fetch(target, parse, err);
    else if (typeof target === "object") return parse(gexf.parse(target, err));
  };
}.call(this));
