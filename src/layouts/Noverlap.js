...
function Noverlap () {
  ...
  this.go = async function () {
    ...
    const pool = Pool(() => spawn(new Worker('./atomicGo')), 8 /* optional size */)
    pool.queue(async atomicGo => {
      const running = await atomicGo(this.running, this.iterCount, this.nodes, window.network, settings)
      console.log(running)
    })
...
function NoverlapUI (props) {
  ...
  const maxIterations = useRef(500)
  ...
  return (
    <div className={'layoutSettings settings'}>
      ...
      <br />
      <div>
        <input step={0.1} min={0.1} max={10} defaultValue={nodeMargin.current} onChange={(event) => {
          nodeMargin.current = event.target.value
        }} type='number' />nodeMargin
      </div>
      <div>
        <input step={0.1} min={0.1} max={10} defaultValue={scaleNodes.current} onChange={(event) => {
          scaleNodes.current = event.target.value
        }} type='number' />scaleNodes
      </div>
      <div>
        <input step={1} min={0} max={10} defaultValue={gridSize.current} onChange={(event) => {
          gridSize.current = event.target.value
        }} type='number' />gridSize
      </div>
      <div>
        <input step={0.1} min={0.1} max={10} defaultValue={permittedExpansion.current} onChange={(event) => {
          permittedExpansion.current = event.target.value
        }} type='number' />permittedExpansion
      </div>
      <div>
        <input step={1} min={0} max={10} defaultValue={rendererIndex.current} onChange={(event) => {
          rendererIndex.current = event.target.value
        }} type='number' />rendererIndex
      </div>
      <div>
        <input step={0.1} min={0.1} max={10} defaultValue={speed.current} onChange={(event) => {
          speed.current = event.target.value
        }} type='number' />speed
      </div>
      <div>
        <input step={0.1} min={0.1} max={10} defaultValue={maxIterations.current} onChange={(event) => {
          maxIterations.current = event.target.value
        }} type='number' />maxIterations
      </div>
      <button onClick={(event) => {
        window.network.configNoverlap({ nodeMargin: nodeMargin.current, easing: 'quadraticInOut', scaleNodes: scaleNodes.current, gridSize: gridSize.current, permittedExpansion: permittedExpansion.current, rendererIndex: rendererIndex.current, speed: speed.current, maxIterations: maxIterations.current })
